/**
 * 模块 07：TypeScript 核心与进阶 - 数据完整性测试
 *
 * 验证模块数据结构、知识点数量、可视化组件覆盖，
 * 并校验模块标准自查文档中"以测促学 / 动态反馈闭环 / 任务导向"等关键检查项。
 */
import { describe, it, expect } from 'vitest'
import { typescriptCoreModule } from '../modules/typescript-core'
import {
  visualizationMeta,
...
describe('TypeScript 核心与进阶 模块', () => {
  it('模块元数据应正确', () => {
    expect(typescriptCoreModule.number).toBe('07')
    expect(typescriptCoreModule.title).toBe('TypeScript 核心与进阶')
    expect(typescriptCoreModule.slug).toBe('typescript-core')
    expect(typescriptCoreModule.stage).toBe('prerequisites')
    expect(typescriptCoreModule.icon).toBe('07')
  })

  it('应包含 19 个知识点', () => {
    expect(typescriptCoreModule.points).toHaveLength(19)
  })

  it('知识点序号应从 1 到 19 连续', () => {
    const orders = typescriptCoreModule.points.map((p) => p.order)
    for (let i = 0; i < 19; i++) {
  ...
  it('每个知识点应有标题、难度和内容块', () => {
    typescriptCoreModule.points.forEach((p) => {
  ...
  it('内容块 id 应在模块内唯一', () => {
    const ids = typescriptCoreModule.points.flatMap((p) =>
      p.blocks.map((b) => b.id),
    )
  ...
  it('知识点难度分布应合理（含易题与难题）', () => {
    const difficulties = typescriptCoreModule.points.map((p) => p.difficulty)
  ...
  it('应包含小测验知识点且题目数 ≥ 10', () => {
    const quizPoint = typescriptCoreModule.points.find(
  ...
  it('应包含知识点速查表（表格形式，≥ 10 行）', () => {
    ...
    const tablePoints = typescriptCoreModule.points.filter(
  ...
  it('应至少包含 2 个带 checks 断言的综合实战沙盒', () => {
    const sandboxPoints = typescriptCoreModule.points.filter(
