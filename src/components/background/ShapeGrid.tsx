/**
 * ShapeGrid - 形状网格背景组件（改编自 React Bits）
 *
 * 在 Canvas 上绘制可移动的形状网格，支持鼠标悬停高亮与拖尾效果。
 * 本项目用作全局背景层，配色遵循项目设计系统：
 * - borderColor: hairline (#212327) 网格线
 * - hoverFillColor: accent-sunset (#ff7a17) 主强调色
 *
 * 默认 props 已按项目需求配置（对角线方向、方形、size=50、speed=0.1、拖尾=15）。
 */
import { useRef, useEffect } from 'react'
import './ShapeGrid.css'

export interface ShapeGridProps {
  /** 网格动画方向：diagonal / up / right / down / left */
  ...
  /** 形状：square / hexagon / circle / triangle */
...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const pushTrail = (next: { x: number; y: number }) => {
